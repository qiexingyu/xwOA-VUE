<template>
  <div class="hello">
      ...
      <div class="search_div">
        ...
        <span class="btn-blue-s sear_btn" v-on:click="findHistoryReim">查询</span>
      </div>
      ...
      <div class="gonggao_body table_body" style="text-align: center;">
        ...
        <div class="data_table_body" style="text-align: center;">
          <ul v-if="findHistoryList.length!=0" v-for="(list, i) in findHistoryList">
          ...
          <ul v-if="findHistoryList.length==0">
             <li class="td7 xh" style="width: 100%;color: red;">暂无审核记录</li>
          </ul>
        </div>
...
export default {
  ...
  created: function () {
    this.findHistoryReim();    
  },
  methods: {        
    findHistoryReim: function(pn, ps){ //查询审核记录
      //pn: 当前页数  ps：每页条数
      ...
      $.ajax({
        url: this.path + "reimbursement/findHistoryReimburseListPage",
        type: "post",
        dataType: "json",
        data: {
        ...
        success: function(data){  
          if(data.code == 200){
            this.findHistoryList = data["data"]["list"];
            this.$refs.childPage.setArg(data["data"]["pages"], this.pageNum, this.pageSize, this.findHistoryReim);
          }else {
