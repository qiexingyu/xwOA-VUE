  ...
  <div class="hello">
      ...
      <div class="gonggao_body table_body">
        <div class="data_table_head">
          <ul>
            ...
            <li class="td14">申请人</li>
            <li class="td14">所属区域</li>
            <li class="td16">花费形式</li>
            <li class="td14">额度</li>
            <li class="td10">详情</li>
            <li class="td13">操作</li>
          </ul>
        ...
        <div class="data_table_body">
          <ul v-for="(l, i) in list" v-if="list.length>0">
            ...
            <li class="td12">{{ l.createTimeString }}</li>
            <li class="td14">{{ l.userName }}</li>
            <li class="td14">{{ l.areaName }}</li>
            <li class="td16">
              {{ l.spendtimeString.slice(0, -3) }}&nbsp;&nbsp;{{ (l.isStatus==1)?'额度调拨-'+costCategory[l.costCategory]:costCategory[l.costCategory] }}
            </li>
            <li class="td14">{{ l.money.toFixed(2) }}</li>
            <li class="td10">
              <a href="javascript:void(0);" class="table_pa" v-on:click="viewPop(l.costId, l.money, l.isStatus, l.costCategory, l.userName)">查看</a>
              <a href="javascript:void(0);" class="table_la" v-on:click="showLeaderYJ(l.reason)" v-if="caiwuLeaderPause">意见</a>
            </li>
            <li class="td13">
              <a href="javascript:void(0);" class="table_pa" v-on:click="agreeApply(l.costId, l.isStatus, l.money, l.costCategory)">通过</a>
              <a href="javascript:void(0);" class="table_la" v-on:click="refuseApply(l.costId)">拒绝</a>
            </li>
          </ul>
      ...
      <div class="com_div" v-if="refuse.ashow">
        ...
        <div class="com_bg">
          ...
          <div class="com_btn">
            <span class="btn-blue-s" v-on:click="refuseSure">确定</span>
            <span class="btn-yellow-s" v-on:click="closeRefuse">取消</span>
          </div>
        </div>
      </div>

      <div class="bgpop" v-if="leaderDo.ashow" v-on:click="canelDB"></div>
      <div class="view_area" style="width:420px;height:466px;overflow-y:auto;" v-if="leaderDo.ashow">
        <span class="closexx" style="margin-right:10px;" v-on:click="canelDB"><Icon type="android-cancel"></Icon></span>
        <div style="font-size: 20px;font-weight: 700;text-align: center;margin-top: 10px">总经理输入审批意见</div>
        <div class="view_eve_table_div">
          <div class="gonggao_body table_body">
            <div class="data_table_head">
              <ul style="height:30px;">
                <li class="xh" style="width:20%;height:30px;line-height:30px;">序号</li>
            ...
            <div class="data_table_body">
              <ul v-for="(u, ui) in leaderDo.appList" style="height:30px;line-height:30px;" v-bind:id="'u'+u.userId">
                <li class="xh" style="width:20%;height:30px;line-height:30px;">{{ ui+1 }}</li>
                <li class="xh" style="width:40%;height:30px;line-height:30px;">{{ u.userName }}</li>
                <li class="xh" style="width:40%;height:30px;line-height:30px;">{{ u.quota?OACommon.formatCurrency(u.quota):OACommon.formatCurrency(u.money) }}</li>
              </ul>
            </div>
          </div>
          ...
          <div style="margin:0 20px;">
            ...
            <textarea placeholder="审批意见最多输入50个字" v-model="leaderDo.appMoney" style="width:241px;" maxlength="50" v-on:blur="leaderBlur"></textarea>
          </div>
        </div>
        <div style="height:20px;text-align:center;color:red;margin-top:5px;">{{ leaderDo.errorTip }}</div>
        <div class="view_btn" style="margin-top:10px;margin-bottom:25px;">
          <span class="btn-blue-s" v-on:click="go2LeaderSP">确定</span>
          <span class="btn-yellow-s" v-on:click="canelDB">取消</span>
        </div>
      </div>

      <div class="bgpop" v-if="approveData.ashow" v-on:click="canelDB"></div>
      <div class="view_area" style="width:420px;height:466px;overflow-y:auto;" v-if="approveData.ashow">
        <span class="closexx" style="margin-right:10px;" v-on:click="canelDB"><Icon type="android-cancel"></Icon></span>
        ...
        <div class="view_eve_table_div">
          <div class="gonggao_body table_body">
            ...
            <div class="data_table_body">
              <ul v-for="u in approveData.appList" style="height:30px;line-height:30px;" v-bind:id="'u'+u.userId" v-bind:num="u.money?u.money:u.quota">
          ...
          <div style="margin:0 40px;">
            调拨额度：<input type="text" placeholder="请输入调拨额度" v-model="approveData.appMoney" v-on:blur="moneyKey"/>
          </div>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      list: [],
      caiwuLeaderPause: false,       //是否是财务领导
      costCategory: {
        1: "个人统筹费用",
        2: "Extra费用",
        3: "区域费用",
      ...
      approveData: {    //财务进行调拨
        ...
        appMoney: '',
        errorTip: '',
        costCategory: 0,  //通过的时候，查看费用类型
      },
      leaderDo: {    //总经理进行审核意见操作
        ashow: false,
        pageNum: 1,
        pageSize: 6,
        appList: [],
  ...
  created: function () {
    ...
    for(var i=0;i<buttonPermissionList.length;i++){
      if(buttonPermissionList[i]["permissionCode"] == "hexiao_caiwu_learder"){
        this.caiwuLeaderPause = true;
        break;
      }
    }
  },
  methods: {
    showLeaderYJ: function(reason){
      this.$refs.childAlert.show(reason);      
    },
    leaderBlur: function(){
      if(this.leaderDo.appMoney){
        this.leaderDo.errorTip = "";
      }
    },
    go2LeaderSP: function(){
      if(!this.leaderDo.appMoney){
        this.leaderDo.errorTip = "请输入审批意见";
        return false;
      }
      this.leaderDo.errorTip = "";
      ...
      $.ajax({
        ...
        data: {
          ...
          "costId": this.costId,
          "action": "1",
          "reason": this.leaderDo.appMoney
        },
        success: function(data){
          if(data["code"] == 200){
            this.leaderDo.ashow = false;
            this.leaderDo.appMoney = '';
            this.leaderDo.errorTip = '';
    ...
    go2DB: function(){
      ...
      //输入金额不能小于申请额度
      if(Number(this.approveData.appMoney) < Number(this.dbMoney)){
        this.approveData.errorTip = "输入金额不能小于申请额度";
        return false;
      }
      //输入金额不能大于选择的人员的额度
      if(Number(this.approveData.appMoney) > Number($("#u"+this.approveData.appModel).attr("num"))){
        this.approveData.errorTip = "输入金额不能大于被调拨人额度";
        return false;
      }
      this.approveData.errorTip = "";
    ...
    moneyKey: function(){
      ...
      var edvalNum = 0;
      if(!reg.test(this.approveData.appMoney)){
        if(!isNaN(this.approveData.appMoney) && this.approveData.appMoney.indexOf(".")!=-1 && this.approveData.appMoney.split(".")[1].length>2){
          edvalNum = this.approveData.appMoney.split(".")[0] + "." + this.approveData.appMoney.split(".")[1].substring(0, 2);
          this.approveData.appMoney = Number(edvalNum).toFixed(2);
        }else{
          this.approveData.appMoney = "0.00";
        }
      }else{
        if(this.approveData.appMoney){
          this.approveData.appMoney = Number(this.approveData.appMoney).toFixed(2);
        }else{
          this.approveData.appMoney = "0.00";
        }
      }
    ...
    agreeApply: function(costId, isStatus, money, costCategory){
      this.dbMoney = money;
      this.costId = costId;
      this.approveData.costCategory = costCategory;
      this.leaderDo.costCategory = costCategory;
        ...
        $.ajax({
          ...
          success: function(data){
            if(data["code"] == 200){   //是财务部的人
              ...
              if(positionList[0]["positionName"] == "总经理"){                   //判断是否总经理
                this.leaderDo.ashow = true;
                this.getUsersByMoneyZJL();
              }else{
                this.costId = costId;
    ...
    getUsersByMoney: function(pn, ps){
      ...
      var moneyUrl = "";
      if(this.approveData.costCategory == 4){    //开课费用的
        moneyUrl = "findQuotaAllcoursePageNum";
      }else{
        moneyUrl = "selectUserByMoney";
      }
      $.ajax({
        url: this.path + moneyUrl,
        type: "post",
        dataType: "json",
        data: {
          "jobNumber": localStorage.getItem("jobNumber"),
    ...
    getUsersByMoneyZJL: function(pn, ps){
      //pn: 当前页数  ps：每页条数
      if(pn && typeof(pn)!='object'){
        this.leaderDo.pageNum = pn;
      }
      if(ps && typeof(ps)!='object'){
        this.leaderDo.pageSize = ps;
      }    

      var moneyUrl = "";
      if(this.leaderDo.costCategory == 4){    //开课费用的
        moneyUrl = "findQuotaAllcoursePageNum";
      }else{
        moneyUrl = "selectUserByMoney";
      }  
      $.ajax({
        ...
        data: {
          "jobNumber": localStorage.getItem("jobNumber"),
          "sessionIdOa": localStorage.getItem("sessionIdOa"),
          "pageNum": this.leaderDo.pageNum,
          "pageSize": this.leaderDo.pageSize
        },
        success: function(data){
          this.leaderDo.appList = data["data"]["list"];

          this.$refs.childPageUser.setArg(data["data"]["pages"], this.leaderDo.pageNum, this.leaderDo.pageSize, this.getUsersByMoneyZJL, "2");
        }.bind(this),
        error: function(){
          console.log("获取销售人员异常");
        }
    ...
    viewPop: function(id, money, isStatus, costCategory, userName){
