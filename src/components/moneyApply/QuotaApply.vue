<template>
  <div class="hello" v-on:click="hideSomePanel">
      ...
      <div class="search_div yydo">
        ...
        <label>费用类型：</label>
        <select v-model="searchData.feeTypeVal" v-on:change="getSelectCostType">
          <option v-for="a in searchData.feeTypeArr" v-bind:value="a.id">{{ a.name }}</option>
        </select>
      </div>

      <div class="gonggao_body">
        <div class="money_table" v-for="(v, k) in tableData" v-if="searchData.feeTypeVal!=4">
          <table cellpadding="0" cellspacing="0" border="1" style="width:98%;">
            <thead>
              <tr>
                <td width="10%" align="center" style="cursor:pointer;" v-on:mouseenter="showProList(k)" v-on:mouseleave="hideProList(k)">
            ...
            <tbody>
              <tr style="height:40px;" v-for="(t, i) in v.sun">
                ...
                <td>
                  {{ (t["c1"]==2)?(t["c13"].join(",")):t["c5"] }}
                  <div class="xiaopen" v-if="dropData.penShow[k].ps" v-on:click.stop="spanShowSchool(i, k)">
                  ...
                  <ul class="person_ul edu" v-if="t.c3==true" style="width:200%;" id="schoolPanelDiv">
                    ...
                    <li v-if="(t.c1!=2)&&(dropData.schoolList.length>0)" v-for="(s, s1) in dropData.schoolList">
                      ...
                      <label v-bind:for="'sc'+s1" style="width:80%;">{{ s.customerName }}</label>
                    </li>
                    <li v-if="(t.c1==2)&&(dropData.schoolList.length>0)" v-for="(s, s1) in dropData.schoolList">
                ...
                <td>
                  {{ (t.c1!=2)?t["c8"]:'N/A' }}
                  <div class="xiaopen" v-if="(t.c1!=2)&&dropData.penShow[k].ps" v-on:click.stop="spanLinkMan(i, k)">
                    <Icon type="compose"></Icon>
                  </div>
        ...
        <div class="money_table" v-for="(v, k) in tableData" v-if="searchData.feeTypeVal==4">
          ...
          <div style="text-align:right;margin-top:10px;width:98%;">
            <span class="btn-yellow-s" v-on:click="removeTableData(k)" v-if="(tableData.length>1) && k!=0">删除</span>
            <span class="btn-blue-s" v-on:click="addNewTableData(k)" style="margin-right:0;" v-if="k==(tableData.length-1)">新增</span>
          </div>
        </div>
      ...
      <div class="view_area" style="width:1024px;height:400px;overflow-y:auto;"
       v-bind:class="{'check': viewData.shCheck}">
        ...
        <div style="margin:20px;border:1px solid #DDD;padding:5px;">
          ...
          <p>额度类型：额度调拨费用&nbsp;&nbsp;{{ viewData.eTypeName }}</p>
          ...
          <table cellpadding="0" cellspacing="0" border="1">
            ...
            <tbody v-for="(vi, vk) in tableData">
              <tr v-for="(vr, vu) in vi.sun">
                ...
                <td v-bind:rowspan="vi.sun.length" v-if="vu==0">{{ (searchData.feeTypeVal==4)?dropData.selectedCourseTypeName[vk]["name"]:'N/A' }}</td>
                <td>{{ vr.c2 }}</td>
                <td>{{ (searchData.feeTypeVal==4)?'N/A':((vr.c1==2)?(vr.c13.join(",")):vr.c5) }}</td>
                <td>{{ (searchData.feeTypeVal==4)?'N/A':((vr.c1!=2)?vr["c8"]:'N/A') }}</td>
                <td>{{ vr.c9.edval }}</td>
                <td>{{ vr.c10 }}</td>
                <td>{{ vr.c11 }}</td>
              </tr>
...
export default {
  ...
  created: function () {
    ...
    for(var c=0;c<buttonPermissionList.length;c++){
      if(buttonPermissionList[c]["permissionCode"] == "apply_area"){
        perFeeTypeArr.push({"id":3, "name": "区域费用"});
      }
      if(buttonPermissionList[c]["permissionCode"] == "apply_person"){
        perFeeTypeArr.push({"id":1, "name": "个人统筹"});
      }
      if(buttonPermissionList[c]["permissionCode"] == "apply_openke"){
        perFeeTypeArr.push({"id":4, "name": "开课费用"});
      }
    }
    this.searchData.feeTypeArr = perFeeTypeArr;
    this.searchData.feeTypeVal = perFeeTypeArr[0]["id"];
  ...
  methods: {
    ...
    getOldData: function(){        //修改重提获取原来的申请数据
      $.ajax({
        ...
        success: function(data){
          ...
          this.searchData.feeTypeVal = data["costCategory"];           
          //条目数据赋值
          ...
          for(var o in newData){
            ...
            if(this.searchData.feeTypeVal == 4){
    ...
    viewPersonApply: function(){           //预览个人统筹费用
      ...
      if(this.searchData.feeTypeVal==4){         //开课费用
        //验证课程类型
      ...
      //预览区域花费类型
      for(var h=0;h<this.searchData.feeTypeArr.length;h++){
        if(this.searchData.feeTypeArr[h].id == this.searchData.feeTypeVal){
          this.viewData.eTypeName = this.searchData.feeTypeArr[h].name;
          break;
        }
      }
      //预览区域赋值
    ...
    subPersonApply: function(){            //提交
      ...
      if(this.costId){                     //重新提交
        subApplyData = {
          ...
          "costCategory": this.searchData.feeTypeVal,
          "money": this.subMoney,
      ...
      $.ajax({
        url: this.path + "insertQuotaAllocation",
        type: "post",
        dataType: "json",
        data: subApplyData,
        success: function(data){
    ...
    getSelectCostType: function(){
      var costTypeUrl = (this.searchData.feeTypeVal == 4)?"selectCostTypeCourse":"selectCostType";

      this.dropData.monTypeList = this.OACommon.selectCostType(costTypeUrl);
      //改变费用类型后清空之前的操作数据
      ...
      this.dropData.provincePause = [{"sp":false}];
      this.dropData.courseTypePause = [{"sp":false}];
      this.dropData.selectedCourseTypeName = [{"name": ''}];
      this.dropData.courseTypeId = [{"id": 0}];
      this.dropData.provinceId = [{"id": 0}];
      this.dropData.selectedProvince = [{"name": ''}];
      this.dropData.monTypePause = [{"mp": false}];
      this.dropData.monTypeModel = [[]];
      this.dropData.penShow = [{"ps": false}];
      //this.dropData.selectSchoolModel.push({"sm": [{"sg":''}]});
      this.dropData.selectSchoolModelDuo = [{"sm": []}];
      this.dropData.linkManModel = [{"link": [{"lk": ''}]}];
      this.ed = [{
            sun: [{
                shCheck: false,
                startTime: '',
