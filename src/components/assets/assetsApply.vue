      ...
      <div class="gonggao_body">
        <div class="fbtong_div new_eve_mk">
          <div class="form_eve">
            ...
            <span style="display:inline-block;width:18%;line-height:30px;">{{ deptName }}</span>
          </div>
          <div class="form_eve">
...
export default {
  ...
  data () {
    return {
      ...
      assetDeptString: localStorage.getItem("deptId"), 
      remark: '',
      subPause: true,
      errTip: ''
    }
  ...
  methods: {
    getOldData: function(){
      $.ajax({
        url: this.path + "Asset/findAssetApplyById",
        type: "post",
        dataType: "json",
        data: {
          "jobNumber": localStorage.getItem("jobNumber"),
          "sessionIdOa": localStorage.getItem("sessionIdOa"),
          "assetApplyId": this.assetId
        },
    ...
    subAssets: function(){
      ...
      $.ajax({
        url: this.path + "Asset/insertAssetApply",
        type: "post",
        dataType: "json",
        data: {
          "jobNumber": localStorage.getItem("jobNumber"),
          ...
          "oldAssetId": this.assetId
        },
        success: function(data){
          this.subPause = true;
          if(data["code"] == 200){
            this.$refs.childAlert.show("提交成功", function(){
              this.$router.push('/mainIndex/assets/assetsApplyProgress');
            });
          }else{
