  ...
  <div class="gonggao_body table_body">
    ...
    <div class="data_table_body">
      <ul v-for="l in cList" v-if="cList.length>=0">
        ...
        <li class="td16">
          <a href="javascript:void(0);" class="table_pa" v-on:click="Pass(l.assetApplyId)">通过</a>
          <a href="javascript:void(0);" class="table_la" v-on:click="reject(l.assetApplyId)">拒绝</a>
        </li>
      </ul>
      ...
      $.ajax({
       ...
       success: function(data){
        ...
        if(data.code == 200){
          this.isShowCover = false;
          this.isShowReject = false;
          this.$refs.childAlert.show("已拒绝",this.getAllCustomer);
        }else {
          this.$refs.childAlert.show(data["msg"]);
        }
