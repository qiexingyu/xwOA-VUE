  ...
  <div class="hello">
	...
	<div class="gonggao_body table_body">
		<div class="data_table_head">
			<ul>
				<li class="td9 xh">添加日期</li>
				<li class="td10" style="margin-left:1%;">资产名称</li>
				<li class="td10">资产类型</li>
				<li class="td10">资产型号</li>
				<li class="td9">资产数量</li>
				<li class="td9">资产单价</li>
				<li class="td16">使用部门</li>
				<li class="td9" style="margin-left:1%;">备注</li>
				<li class="td16">操作</li>
			</ul>
		</div>
		<div class="data_table_body">
			<ul v-for="l in cList" v-if="cList.length>=0">
				<li class="td9 xh">{{ OACommon.getOADate(new Date(l.createTime)).todayDate }}</li>
				<li class="td10" style="margin-left:1%;">{{ l.assetName }}</li>
				<li class="td10">{{ l.assetTypeName }}</li>
				<li class="td10">{{ l.assetModel }}</li>
				<li class="td9">{{ l.assetNumber }}</li>
				<li class="td9">{{ l.assetOnePrice?l.assetOnePrice.toFixed(2):'0.00' }}</li>
				<li class="td16"><span v-for="(d, di) in l.deptList">{{ d.dept_name }}<span v-if="di!=(l.deptList.length-1)">,</span></span></li>
				<li class="td9" style="margin-left:1%;">
					<a class="table_pa" v-on:click="viewRemark(l.remark)">查看</a>
				</li>
				<li class="td16">
					<a v-bind:href="'/#/mainIndex/assets/addAssets?id='+l.assetId" class="table_pa">修改</a>
					<a href="javascript:void(0);" class="table_la" v-on:click="deleteAssets(l.assetId)">删除</a>
				</li>
			</ul>
  ...
  methods: {
  	deleteAssets: function(id){
  		this.$refs.childConfirm.show("您确定要删除吗？", function(){
  			$.ajax({
  				url: this.path + "Asset/deleteAssetByAssetId",
  				type: "post",
  				dataType: "json",
  				data: {
  	...
  	getAllCustomer: function(pn, ps){
  		...
  		$.ajax({
	  		url: this.path + "Asset/findAssetPageListByCondition",
		    type: 'post',
		    dataType: 'json',
		    data: {
		      "jobNumber": localStorage.getItem("jobNumber"),
